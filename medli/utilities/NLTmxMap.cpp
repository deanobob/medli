...
NLTmxMap* NLLoadTmxMap( char *xml )
{
    ...
    if ( propertiesnode ) {
        ...
        while ( propertynode ) {
            std::string name = propertynode->first_attribute( "name" )->value();
    ...
    while ( tilesetnode ) {
        ...
        if (imagenode) {
          tileset->filename = imagenode->first_attribute( "source" )->value();
          tileset->imageWidth = atoi( imagenode->first_attribute( "width" )->value() );
          tileset->imageHeight = atoi( imagenode->first_attribute( "height" )->value() );
        }
